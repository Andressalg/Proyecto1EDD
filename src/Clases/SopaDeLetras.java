...
public class SopaDeLetras {
    ...
    private void bfs(Cola cola, boolean[][] visitados, ListaSimplementeEnlazada palabrasEncontradas) {
        while (!cola.isEmpty()) {
            NodoCola nodoCola = (NodoCola) cola.desencolar();
            NodoBFS nodoActual = new NodoBFS((PalabraConPosicion) nodoCola.getData());
            ...
            String sb = palabraActual.getPalabra();

            // Agregar la palabra actual a la lista de palabras encontradas
            palabrasEncontradas.AddAtEnd(palabraActual);
                ...
                for (int j = -1; j <= 1; j++) {
                    if (i != 0 || j != 0) {
                        ...
                        if (nuevaFila >= 0 && nuevaFila < Global.getMatriz().getNumFilas() && nuevaColumna >= 0 && nuevaColumna < Global.getMatriz().getNumColumnas() && !visitados[nuevaFila][nuevaColumna]) {
                            Palabra nuevaPalabra = new Palabra(sb + Global.getMatriz().getMatrizAdyacencia()[nuevaFila][nuevaColumna].getLetra());
                            ...
                            try {
                                ...
                                cola.encolar(new NodoCola(nuevoNodo));
                            }
                        }
                    }
                }
